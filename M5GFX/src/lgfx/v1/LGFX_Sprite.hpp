...
//      _bitwidth = 0;
    ...
    LGFX_INLINE void pushSprite(                int32_t x, int32_t y, epd_wipe_direction_t wipe_dir) { push_sprite(_parent, x, y, pixelcopy_t::NON_TRANSP, wipe_dir); }
    LGFX_INLINE void pushSprite(LovyanGFX* dst, int32_t x, int32_t y, epd_wipe_direction_t wipe_dir) { push_sprite(    dst, x, y, pixelcopy_t::NON_TRANSP, wipe_dir); }
    template<typename T>
    LGFX_INLINE void pushSprite(                int32_t x, int32_t y, const T& transp, epd_wipe_direction_t wipe_dir) { push_sprite(_parent, x, y, _write_conv.convert(transp) & _write_conv.colormask, wipe_dir); }
    template<typename T>
    LGFX_INLINE void pushSprite(LovyanGFX* dst, int32_t x, int32_t y, const T& transp, epd_wipe_direction_t wipe_dir) { push_sprite(    dst, x, y, _write_conv.convert(transp) & _write_conv.colormask, wipe_dir); }
...
//----------------------------------------------------------------------------
  ...
  protected:
    ...
    void push_sprite(LovyanGFX* dst, int32_t x, int32_t y, uint32_t transp = pixelcopy_t::NON_TRANSP, epd_wipe_direction_t wipe_dir = epd_wipe_direction_t::epd_wipe_default)
    {
      if (wipe_dir != epd_wipe_direction_t::epd_wipe_default) {
        dst->setEpdWipeDirection(wipe_dir, true);
      }
